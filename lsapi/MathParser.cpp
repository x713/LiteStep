...
struct { const wchar_t *name; MathFunction function; unsigned int numArgs; } gFunctions[] =
{
    { L"abs",        Math_abs,        1 },
...
MathValue MathParser::GetVariable(const wstring& name) const
{
    ...
    if (mRecursiveVarSet.count(name) > 0)
    {
        ...
        StringCchPrintf(message, MAX_LINE_LENGTH, L"Error: Variable \"%s\" is defined recursively.", name.c_str());
...
MathValue MathParser::ParsePrimaryExpression()
{
    ...
    else if (mLookahead[0].GetType() == TT_ID)
    {
        ...
        if ((mFlags & MATH_EXCEPTION_ON_UNDEFINED) && value.IsUndefined())
        {
            ...
            wostringstream message;
            message << L"Error: Variable " << name << L" is not defined.";
            throw MathException(message.str());
        }
...
{
    return (argList[0].ToString().find(argList[1].ToString()) != wstring::npos);
}
